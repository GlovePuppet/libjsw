...
#include "images/icon_link_hidden_20x20.xpm"
...
#define FILE_BROWSER_KEY		"/FileBrowser"
...
typedef enum {
	...
	FB_ICON_EXECUTABLE,
	FB_ICON_DRIVE_FIXED,
	FB_ICON_DRIVE_ROOT			/* 17 */
} FileBrowserIconIndex;


/* Utilities */
static gchar **FileBrowserListDirectory(
	const gchar *path,
	gint *nobjects_rtn
);
static gboolean FileBrowserIsNameHidden(const gchar *name);
static gboolean FileBrowserIsPathDirectory(const gchar *path);
static gboolean FileBrowserIsLPathDirectory(const gchar *path);
static const gchar *FileBrowserGetExtensionFromPath(const gchar *path);
static gchar *FileBrowserJoinPaths(
...
static gboolean FileBrowserMatchName(
	const gchar *name,
	const gchar *extensions
);
static gint FileBrowserDirectoryCreate(const gchar *path);
static gint FileBrowserLinkCreate(
	const gchar *path,
	const gchar *target
);
static gint FileBrowserUnlink(const gchar *path);
...
static FileBrowserIconIndex FileBrowserMatchIconNumFromPath(
	FileBrowser *fb,
	const gchar *path,
...
static void FileBrowserDestroyCB(gpointer data);
static gint FileBrowserDeleteEventCB(
	GtkWidget *widget, GdkEvent *event, gpointer data
);
static void FileBrowserRealizeCB(GtkWidget *widget, gpointer data);
...
/* File Selection Dialog Front Ends */
...
gboolean FileBrowserGetResponse(
	...
	const gchar *path,
	fb_type_struct **file_types_list, gint nfile_types,
...
/* File Types List */
fb_type_struct *FileBrowserTypeNew(void);
void FileBrowserTypeDelete(fb_type_struct *t);
...
gchar *FileBrowserTypeCompleteExtension(
	const gchar *path,
	fb_type_struct *t,
...
#define FB_DEF_TITLE			"Select File"

#define FB_DEF_TYPE_STR			"All Files (*.*)"
 ...
 *	Icons Data List:
 *
 *	The order of this list must correspond with FileBrowserIconIndex.
 *
...
struct _FileBrowser {
	...
	fb_type_struct	*current_file_type;

	/* Selected Paths Return */
	gchar 		**selected_paths_list;
	gint		nselected_paths;
...
/*
 *	Gets the complete list of objects (including special directory
 *	notations) in a directory.
 *
 *	The path specifies the directory.
 *
 *	The nobjects_rtn specifies the return value for the number of
 *	objects in the directory.
 *
 *	Returns a list of dynamically allocated strings describing the
 *	name of each object in the directory.
 */
static gchar **FileBrowserListDirectory(
	const gchar *path,
	gint *nobjects_rtn
...
/*
 *	Checks if the name is considered hidden.
 */
static gboolean FileBrowserIsNameHidden(const gchar *name)
{
	if(name == NULL)
	...
	/* Check if it is a special directory notation which might be
	 * confused as hidden
	 */
	if((name[1] == '\0') || (name[1] == '.'))
		return(FALSE);
...
/*
 *	Checks if the path's destination refers to a directory.
 */
static gboolean FileBrowserIsPathDirectory(const gchar *path)
{
 ...
 *
 *	The parent_path specifies the string describing the full path
 *	to the parent. The parent may not be NULL and must be a full
 *	path.
 *
 *	The child_path specifies the string describing the child path.
 *	If child_path is NULL then a copy of parent_path is returned.
 *	If child_path is a full path then a copy of child_path is
 *	returned. Otherwise a copy of the parent_path prefixed to the
 *	child_path is returned.
 *
...
{
	return(STRDUP(PrefixPaths(parent_path, child_path)));
}
...
/*
 *	Checks if the name matches a filter.
 *
 *	The name specifies the name to test.
 *
 *	The extensions specifies a string describing a space-separated
 *	list of extensions, each extension will be used as a filter to
...
)
{
	...
	while(ISBLANK(*extensions) || (*extensions == ','))
		extensions++;
	...
	while(*extensions != '\0')
	{
		...
		/* Copy the current extension from the extensions list and
		 * then to the next position
		 */
		cur_ext_len = 0;
		st = cur_ext;
		st_end = cur_ext + 79;	/* Set end 1 character premature */
		while((st < st_end) && !ISBLANK(*extensions) && (*extensions != ',') &&
			  (*extensions != '\0')
		)
		{
			/* If there are any wildcards in this extension then we
			 * need to use fnmatch() for this extension
			 */
		...
		*st = '\0';
		...
		/* Seek to the next extension in the list */
		while(ISBLANK(*extensions) || (*extensions == ','))
			extensions++;
...
#if defined(_WIN32)
	return((gint)mkdir((const char *)path));
#else
	return((gint)mkdir(
		(const char *)path,
		(mode_t)(~m) &
...
)
{
	...
	return((gint)symlink(
		(const char *)target,		/* Old path (target) */
		(const char *)path			/* New path (link) */
	));
}
...
/* TODO Get a list of all drives */
/*
		if(stat((const char *)drive_name, &stat_buf))
			continue;
 */
...
/*
 *	Sets the File Selection Dialog busy or ready.
 */
static void FileBrowserSetBusy(
	FileBrowser *fb,
	const gboolean busy
...
)
{
	...
	switch(list_format)
	{
	  case FB_LIST_FORMAT_VERTICAL:
		...
		GUIMenuItemSetCheck(
			fb->list_format_standard_micheck,
			FALSE,
			FALSE
		);
		GTK_TOGGLE_BUTTON_SET_ACTIVE(
		...
		GUIMenuItemSetCheck(
			fb->list_format_vertical_micheck,
			TRUE,
			FALSE
		);
		GTK_TOGGLE_BUTTON_SET_ACTIVE(
		...
		GUIMenuItemSetCheck(
			fb->list_format_vertical_details_micheck,
			FALSE,
			FALSE
		);
		break;
...
)
{
	 ...
	 */
	if(!FileBrowserIsPathDirectory(new_location))
	{
	...
	if((info == 0) || (info == 1) || (info == 2))
	{
		...
		if(widget == fb->list_da)
		{
			...
			if(paths_list != NULL)
			{
				...
				if(o != NULL)
				{
					tar_path = STRDUP(o->path);
					if(!FileBrowserIsPathDirectory(tar_path))
					{
						g_free(tar_path);
...
#if defined(_WIN32)
#define GET_ICON(_path_)	{		\
 if(stat((const char *)(_path_), &stat_buf))	\
  icon = NULL;					\
 else						\
...
#else
#define GET_ICON(_path_)	{		\
 if(lstat((const char *)(_path_), &stat_buf))	\
  icon = NULL;					\
 else						\
  icon = FileBrowserGetIcon(			\
   fb,						\
...
/* Copies a shortened string as s_shortened */
#define STRING_COPY_SHORTEN(_s_) {		\
 if((_s_) != NULL) {				\
  const gint len = strlen(_s_),			\
		 max_characters = FB_LOC_LIST_MAX_CHARS;\
											\
  /* Does the string need to be shortened? */	\
  if(len > max_characters)			\
   s_shortened = g_strconcat(			\
    "...",					\
    &(_s_)[len - max_characters + 3],		\
    NULL					\
   );						\
  else						\
   s_shortened = g_strdup(_s_);			\
 } else {					\
  s_shortened = NULL;				\
 }						\
}
	...
	{
		gint	i;
		gchar	*path = g_strdup(cur_location),
					*path_ptr,
					*s_shortened;
		FileBrowserIconIndex icon_num;

#if defined(_WIN32)
		/* Get the icon for the current location */
		if(stat((const char *)path, &stat_buf))
			icon_num = FB_ICON_FOLDER_CLOSED;
		else
		...
		PUListSetItemDataFull(
			pulist,
			i,
			g_strdup(path), destroy_cb
		);

		/* Get each parent location */
		path_ptr = (gchar *)strrchr(
			(char *)path,
			G_DIR_SEPARATOR
		);
		if(path_ptr == NULL)
			path_ptr = path;
#if defined(_WIN32)
		while(path_ptr > (path + 3))
#else
		while(path_ptr > path)
#endif
		{
			/* Terminate the path string at its parent deliminator
			 * character so that path now describes the parent
			 * path
			 */
			*path_ptr = '\0';
		...
		path_ptr = (gchar *)strchr((const char *)path, G_DIR_SEPARATOR);
		if(path_ptr != NULL)
		{
			if((*(path_ptr + 1) != '\0')
			{
...
)
{
	...
	if(path != NULL)
	{
		...
		if(s != NULL)
		{
			if(!strcmp((const char *)s, (const char *)path))
				return(FB_ICON_FOLDER_HOME);
		}

		/* Toplevel? */
		if(!g_strcasecmp(path, "/"))
		...
		ext = FileBrowserGetExtensionFromPath(path);
	}
	else
	{
		name = NULL;
...
#endif
	{
		...
		if(fb->euid != 0)
		{
			...
			/* Hidden? */
			if(FileBrowserIsNameHidden(name))
		...
		{
			/* Root always owns the object, so check if the owner
			 * has access
			 */
			if(!(m & S_IXUSR))
	...
	if(S_ISLNK(m))
	{
		...
		if(stat((const char *)path, &stat_buf))
		{
			return(FB_ICON_LINK_BROKEN);
		}
		/* Hidden Link? */
		if(FileBrowserIsNameHidden(name))
		{
			return(FB_ICON_LINK_HIDDEN);
		}
		return(FB_ICON_LINK);
	}
#endif
...
)
{
	...
	/* Get the list's GtkStyle and GdkFont */
	w = fb->list_da;
	style = (w != NULL) ? gtk_widget_get_style(w) : NULL;
	font = (style != NULL) ? gtk_style_get_font(style) : NULL;

	/* Get the icon index based on the object's path and statistics */
...
)
{
	...
	t_cur = fb->current_file_type;

	/* Show hidden objects? */
	show_hidden_objects = (fb->flags & FB_SHOW_HIDDEN_OBJECTS) ? TRUE : FALSE;

	/* Get the extensions list from the filter (if specified) or
	 * from the current file type
	 */
	extensions = (filter != NULL) ? filter : t_cur->extensions;
	if(extensions != NULL)
		match_all_files = (!strcmp(extensions, "*.*") || !strcmp(extensions, "*")) ? TRUE : FALSE;
	else
	...
	if(cur_location != NULL)
	{
		...
		gint nobjs;
		gchar **names_list = FileBrowserListDirectory(
			cur_location,
			&nobjs
		);
		if(names_list != NULL)
...
/*
 *	Redraws the File Selection Dialog's list header.
 */
static void FileBrowserListHeaderDraw(FileBrowser *fb)
{
	gint		width, height,
	...
	if(fb->columns_list != NULL)
	{
		...
		)
		{
			...
			/* Draw the frame for this heading */
			gtk_paint_box(
				style,
				(GdkWindow *)drawable,
...
/*
 *	Queues a draw of the File Selection Dialog's list header.
 */
static void FileBrowserListHeaderQueueDraw(FileBrowser *fb)
{
	if(fb == NULL)
...
/*
 *	Draws the File Selection Dialog's list.
 */
static void FileBrowserListDraw(FileBrowser *fb)
{
	gint		scroll_x = 0, scroll_y = 0,
		...
		if(g_list_length(fb->columns_list) >= 4)
		{
			...
			)
			{
				...
				if(S_ISREG(m))
				{
					...
					gchar *s_shortened = strchr(s, '\n');
					if(s_shortened == NULL)
						s_shortened = strchr(s, '\r');
					if(s_shortened != NULL)
						*s_shortened = '\0';
					gdk_string_bounds(font, s, &b);
	...
	if((GTK_WIDGET_HAS_FOCUS(w) &&
		GTK_WIDGET_SENSITIVE(w)
	   ) || (drag_flags & FILE_BROWSER_DRAG_OVER)
	)
		gtk_paint_focus(
			style,
			(GdkWindow *)drawable,
			GTK_STATE_NORMAL,				/* Entire area */
			NULL,
			w,
			NULL,				/* No detail */
			0, 0,
...
/*
 *	Queues a redraw of the File Selection Dialog's list.
 */
static void FileBrowserListQueueDraw(FileBrowser *fb)
{
	if(fb == NULL)
...
)
{
	...
	for(i = 0; i < total; i++) 
	{
		...
		t2->extensions = STRDUP(t->extensions);
		t2->name = STRDUP(t->name);

		if(STRISEMPTY(t2->name))
			label = STRDUP(t2->extensions);
		else
		...
		PUListSetItemDataFull(
			pulist,
			n,
			t2, (GtkDestroyNotify)FileBrowserTypeDelete
		);
	}
...
#define MESSAGE_MKDIR_FAILED(_msg_)	{	\
 ...
 CDialogSetTransientFor(NULL);			\
	...
	if(FileBrowserDirectoryCreate(path))
	{
		...
		/* Try creating the new directory under a different name */
		for(i = 2; i < 100; i++)
		{
			/* Regenerate a different new name and full path */
			g_free(name);
			g_free(path);
			...
			/* Try to create the new directory with this new name */
			if(!FileBrowserDirectoryCreate(path))
			{
				created = TRUE;
				break;
			...
			/* Unable to create the directory for reasons other than
			 * it already existed?
			 */
...
)
{
...
#endif
	{
		...
		if(FileBrowserDirectoryCreate(new_path))
			return(-1);
		...
		status = 0;
		names_list = FileBrowserListDirectory(
			old_path,
			&nnames
		);
		if(names_list != NULL)
		{
			...
			for(i = 0; i < nnames; i++)
			{
				...
				old_child_path = FileBrowserJoinPaths(
					old_path,
					name
				);
				new_child_path = FileBrowserJoinPaths(
					new_path,
					name
				);
...
#endif
	{
		const size_t file_size = lstat_buf.st_size;
		size_t	io_buf_len,
			coppied_size;
#if !defined(_WIN32)
		struct utimbuf utime_buf;
		...
		io_buf_len = 1024l;
#else
		io_buf_len = (size_t)lstat_buf.st_blksize;
#endif
...
#if !defined(_WIN32)
			if((size_t)tar_stat_buf.st_blksize < io_buf_len)
				io_buf_len = (size_t)tar_stat_buf.st_blksize;
#endif
		}
		if(io_buf_len < 1l)
			io_buf_len = 1l;
...
deliminators which are not allowed in an object's name"
	...
	o = FileBrowserGetObject(fb, i);
	if((o != NULL) ?
		(o->name != NULL) && (o->path != NULL) : FALSE
	)
	{
		struct stat lstat_buf;
		const gchar *cur_location = FileBrowserGetLocation(fb);
		gchar	*old_full_path = STRDUP(o->path),
					*new_full_path = FileBrowserJoinPaths(
						cur_location,
						value
					);
...
#ifdef S_ISLNK
	/* Permission of links may not be changed */
	if(S_ISLNK(m))
	{
		CDialogSetTransientFor(toplevel);
		CDialogGetResponse(
...
#if !defined(S_IRUSR) || !defined(S_IWUSR) || !defined(S_IXUSR)
	/* Since permissions are not supported we cannot chmod */
	if(TRUE)
	{
		CDialogSetTransientFor(toplevel);
		CDialogGetResponse(
...
)
{
	...
	if(FileBrowserIsLPathDirectory(path))
	{
		gint	status = 0,
			nnames;
		...
		if(names_list != NULL)
		{
			...
			for(i = 0; i < nnames; i++)
			{
				...
				/* Get the full path to this child object */
				child_path = FileBrowserJoinPaths(
					path,
					name
				);
				if(child_path != NULL)
...
static void FileBrowserEntryActivate(FileBrowser *fb)
{
	...
	else
	{
		...
		{
			const gchar *cur_location = FileBrowserGetLocation(fb);
			full_path = FileBrowserJoinPaths(
				cur_location,
				v
			);
		}
 ...
 *	the File Selection Dialog and breaks out of any main levels.
 */
static void FileBrowserOK(FileBrowser *fb)
{
	if(fb == NULL)
	...
	else
	{
		...
		/* Delete the previous list of selected paths */
		for(i = 0; i < fb->nselected_paths; i++)
			g_free(fb->selected_paths_list[i]);
		g_free(fb->selected_paths_list);
		fb->selected_paths_list = NULL;
		fb->nselected_paths = 0;
...
/*			name = g_strstrip(name); */
					 ...
					 */
					n = MAX(fb->nselected_paths, 0);
					fb->nselected_paths = n + 1;
					fb->selected_paths_list = (gchar **)g_realloc(
						fb->selected_paths_list,
						fb->nselected_paths * sizeof(gchar *)
					);
					if(fb->selected_paths_list != NULL)
					{
						/* If name is an absolute path then copy
						 * it to the selected_path list
						 *
						 * Otherwise prefix the current location to
						 * the name
						 */
						if(g_path_is_absolute(name))
						{
							fb->selected_paths_list[n] = g_strdup(name);
						}
						else if(*name == '~')
						{
							/* Prefix the home directory to the new location path */
							const gchar	*home = fb->home_path,
											*child = name + 1;
							fb->selected_paths_list[n] = g_strconcat(
								(home != NULL) ? home : "",
								(*child != G_DIR_SEPARATOR) ? G_DIR_SEPARATOR_S : "",
								child,
								NULL
						...
						{
							fb->selected_paths_list[n] = FileBrowserJoinPaths(
								cur_location,
								name
							);
						}
		    ...
		    else
		    {
				 ...
				 */
				const gchar *cur_location = FileBrowserGetLocation(fb);
				gint n = MAX(fb->nselected_paths, 0);
				fb->nselected_paths = n + 1;
				fb->selected_paths_list = (gchar **)g_realloc(
				    fb->selected_paths_list,
				    fb->nselected_paths * sizeof(gchar *)
				...
				{
				    fb->selected_paths_list[n] = g_strdup(cur_location);
				}
		    }
		}
 ...
 *	breaks out of those queries.
 *
 *	Otherwise, the File Selection Dialog is unmapped and any main levels
...
static void FileBrowserDestroyCB(gpointer data)
{
	...
	FileBrowserTypeDelete(fb->current_file_type);
	fb->current_file_type = NULL;
...
static void FileBrowserRealizeCB(GtkWidget *widget, gpointer data)
{
	...
	/* Do not mark the File Selection Dialog as realized here,
	 * FileBrowserListRealizeCB() will mark it as realized
	 */
}
...
static void FileBrowserListRealizeCB(GtkWidget *widget, gpointer data)
{
	...
	/* Mark the List GtkDrawingArea as realized */
	  ...
	  case GDK_Tab:
		...
		if(key_press)
		{
			...
			if(path != NULL)
			{
				 ...
				 */
				if(!g_path_is_absolute(path) && (*path != '~'))
				{
					if(STRISEMPTY(path))
					{
						g_free(path);
					...
					{
						gchar *path2 = FileBrowserJoinPaths(
							FileBrowserGetLocation(fb),
							path
						);
						g_free(path);
						path = path2;
...
)
{
	...
	g_free(t_cur->extensions);
	t_cur->extensions = STRDUP(t_sel->extensions);

	g_free(t_cur->name);
	t_cur->name = STRDUP(t_sel->name);
...
/*
 *	File Selection Dialog ok button signal callback.
 */
static void FileBrowserOKCB(GtkWidget *widget, gpointer data)
{
...
/*
 *	File Selection Dialog cancel button signal callback.
 */
static void FileBrowserCancelCB(GtkWidget *widget, gpointer data)
{
	FileBrowserCancel(FILE_BROWSER(data));
...
gint FileBrowserInit(void)
{
	...
	/* Initialize the Floating Prompt as needed */
	if(FPromptInit())
	{
		g_printerr(
"FileBrowserInit() Unable to initialize the Floating Prompt.\n"
		);
		return(-1);
	}

	/* Initialize the Confirmation Dialog as needed */
	if(CDialogInit())
	{
		g_printerr(
"FileBrowserInit() Unable to initialize the Confirmation Dialog.\n"
		);
		return(-1);
	}

	/* Create a new File Selection Dialog */
	file_browser = fb = FILE_BROWSER(g_malloc0(sizeof(FileBrowser)));
	if(fb == NULL)
		return(-3);
 ...
 */
	...
	fb->last_single_select_object = -1;
/*	fb->selection = fb->selection_end = NULL; */
	fb->current_file_type = FileBrowserTypeNew();
/*
	fb->selected_paths_list = NULL;
	fb->nselected_paths = 0;
 ...
 */
	...
	gtk_object_set_data_full(
		GTK_OBJECT(w), FILE_BROWSER_KEY,
		fb, FileBrowserDestroyCB
	);
#ifdef PROG_NAME
...
#endif
	gtk_widget_set_name(w, FILE_BROWSER_TOPLEVEL_WIDGET_NAME);
	...
	gtk_signal_connect(
		...
		GTK_SIGNAL_FUNC(FileBrowserRealizeCB), fb
	);
...
"Detailed Listing"
	...
	gtk_widget_set_name(w, FILE_BROWSER_LIST_HEADINGS_WIDGET_NAME);
	...
	gtk_widget_set_name(w, FILE_BROWSER_CONTENTS_LIST_WIDGET_NAME);
...
/*	    fb->refresh_mi = w; */
		...
		if(fb->flags & FB_SHOW_HIDDEN_OBJECTS)
			GUIMenuItemSetCheck(w, TRUE, FALSE);
...
"File Type:"
	...
	i = PUListAddItem(pulist, FB_DEF_TYPE_STR);
	if(i > -1)
	{
		...
		if(t != NULL)
		{
			...
			PUListSetItemDataFull(
				pulist,
				i,
				t, (GtkDestroyNotify)FileBrowserTypeDelete
			);
		}
...
/*
 *	Sets the File Selection Dialog's style.
 */
void FileBrowserSetStyle(GtkRcStyle *rc_style)
{
	GtkWidget *w;
...
/*
 *	Sets the File Selection Dialog to be a transient for the given
 *	GtkWindow w.
 *
 *	If w is NULL then the transient for will be unset.
 */
void FileBrowserSetTransientFor(GtkWidget *w)
{
	FileBrowser *fb = file_browser;
...
/* Since the File Selection Dialog itself has popup windows that may need to be
 * set modal, we cannot set the File Selection Dialog itself modal.
		gtk_window_set_modal(
			GTK_WINDOW(fb->toplevel),
			TRUE
		);
 */
...
/*
 *	Returns the File Selection Dialog's toplevel GtkWidget.
 */
GtkWidget *FileBrowserGetToplevel(void)
{
	FileBrowser *fb = file_browser;
...
/* Do not reset the current location */
	...
	/* Clear the file name GtkEntry */
	gtk_entry_set_text(GTK_ENTRY(fb->entry), "");
...
/*
 *	Maps the File Selection Dialog and sets up the inital values.
 *
...
#define RESET_RETURNS	{		\
 if(paths_list_rtn != NULL)		\
  *paths_list_rtn = NULL;		\
 if(npaths_rtn != NULL)			\
  *npaths_rtn = 0;			\
									\
 if(file_type_rtn != NULL)		\
  *file_type_rtn = NULL;		\
}
	...
	/* Set the types list */
	if((file_types_list != NULL) && (nfile_types > 0))
	...
	/* Unmap the File Selection Dialog just in case it was not unmapped
	...
	/* Selected paths list */
	if(paths_list_rtn != NULL)
		*paths_list_rtn = fb->selected_paths_list;
	if(npaths_rtn != NULL)
		*npaths_rtn = fb->nselected_paths;
...
/*
 *	Maps the File Selection Dialog.
 */
void FileBrowserMap(void)
{
	FileBrowser *fb = file_browser;
	...
	gtk_widget_grab_focus(fb->list_da);
}

/*
 *	Unmaps the File Selection Dialog.
 */
void FileBrowserUnmap(void)
{
	FileBrowser *fb = file_browser;
...
/*
 *	Shuts down the File Selection Dialog.
 */
void FileBrowserShutdown(void)
{
	FileBrowser *fb = file_browser;
	 ...
	 * FileBrowserDestroyCB() to delete the rest of the File
	 * Browser
	 */
...
const gchar *FileBrowserGetLastLocation(void)
{
	...
	return(FileBrowserGetLocation(fb));
}
...
fb_type_struct *FileBrowserTypeNew(void)
{
	return(FB_TYPE(g_malloc0(sizeof(fb_type_struct))));
}
...
/*
 *	Appends a new file type to the file types list.
 *
 *	The list and total specifies the current and return value for
 *	the file types list.
 *
 *	The extensions specifies a string describing a space-separated
 *	list of extensions, each extension must include the '.'
 *	character, the first extension is considered the default
 *	extension if there are multiple extensions in the list.
 *
 *	The name specifies a verbose name/description of the extension.
 *
 *	Returns the index of the newly appended file type in the list
...
)
{
	...
	/* Increase the list's allocation */
	i = MAX(*total, 0);
	*total = i + 1;
	*list = (fb_type_struct **)g_realloc(
	...
	/* Set the new file type's values */
	t->extensions = STRDUP(extensions);
	t->name = STRDUP(name);
...
/*
 *	Deletes the file types list.
 *
 *	The list and total the file types list.
 */
void FileBrowserDeleteTypeList(
	fb_type_struct **list, const gint total 
)
...
/*
 *	Appends the default (first) extension to the path.
 *
 *	The path specifies the path to append or replace the extension
 *	to.
 *
 *	The t specifies the file type containing the list of extensions,
 *	only the default (first) extension will be used.
 *
 *	If always_replace is TRUE then if the specified path already
 *	has an extension and it is different then it will be replaced
 *	with the default (first) extension in the file type.
 *
 *	Returns a copy of path with the extension appended or replaced.
 */
gchar *FileBrowserTypeCompleteExtension(
	const gchar *path,
	fb_type_struct *t,
...
)
{
	...
	/* Get the space-separated list of extensions */
	extensions = t->extensions;
	if(extensions == NULL)
		return(g_strdup(path));

	/* If the specified extension specifies all types then just
	...
	/* Explode the space-separated list of extensions into a list
	 * of single extension strings
	 */
	extensions_list = g_strsplit(
		extensions,
		" ",
		-1
	);
	...
	/* Get the default (first) extension and "clean" it by removing
	 * any ',' characters and seeking past any wildcards
	 */
	ext_default = extensions_list[0];
	if(ext_default != NULL)
	{
		/* Seek to the '.' character */
		gchar *s = (gchar *)strchr((char *)ext_default, '.');
		if(s != NULL)
			ext_default = s;

		/* Remove all ',' characters */
		substr((char *)ext_default, ",", "");
	}

	/* Get the specified path's extension */
	ext = FileBrowserGetExtensionFromPath(path);

	/* Does the specified path not have an extension? */
	if(ext == NULL)
	{
		...
		if(ext_default != NULL)
		{
			...
			g_strfreev(extensions_list);
			return(path_rtn);
		}
		else
		{
			/* No default extension so just return a copy of the path */
			path_rtn = g_strdup(path);
			g_strfreev(extensions_list);
			return(path_rtn);
		}
	}

	/* Check if the path's extension is not in the list */
	if(strstr((const char *)extensions, (const char *)ext) == NULL)
	{
		gint i;
		gchar *path_without_ext;

		/* If we do not need to replace the path's extension then
		 * just return a copy of the path now
		 */
		if(!always_replace)
		{
			g_strfreev(extensions_list);
		...
		/* Copy the path and remove this current extension */
		path_without_ext = g_strdup(path);
		...
		i = (gint)strlen(path_without_ext) - (gint)strlen(ext);
		if(i >= 0)
			path_without_ext[i] = '\0';

		/* Append the default extension to the path */
		if(ext_default != NULL)
		{
			path_rtn = g_strconcat(
				path_without_ext,
	...
	/* The specified path has an extension and it is in the list so
	 * just return a copy of the path
	 */
	path_rtn = g_strdup(path);
