 ...
 */
typedef struct {
	GtkWidget	*invisible;		/* To grab (steal) GdkEvents
											 * so that the blocked
											 * GtkWidget does not get them  */
	GtkWidget	*w;			/* The GtkWidget being blocked */
	gint		level;
} GUIBlockInputData;
...
#define GUI_SELECTION_DATA(p)		((GUISelectionData *)(p))
#define GUI_SELECTION_DATA_KEY		"GUI/Selection/Data"
...
typedef struct {
	...
	guint		selection_clear_event_sigid,
					selection_get_sigid;
	GList		*datas_list;		/* GList of GUISelectionData * */
} GUISelectionOwner;
#define GUI_SELECTION_OWNER(p)		((GUISelectionOwner *)(p))
#define GUI_SELECTION_OWNER_KEY		"GUI/Selection/Owner"
...
typedef struct {
	...
	gint		ref_count;
	GtkWidget	*icon_pm;		/* Icon GtkPixmap */
	gint		x,			/* Hot spot */
					y,
					width,			/* Size */
					height;
} GUIDNDDragIcon;
#define GUI_DND_DRAG_ICON(p)		((GUIDNDDragIcon *)(p))
#define GUI_DND_DRAG_ICON_KEY		"GUI/DND/DragIcon"
...
typedef struct {
	GtkWidget	*menu_item,
					*pixmap,		/* Icon GtkPixmap */
					*label,			/* Main GtkLabel */
					*accel_label;		/* Accelkey GtkLabel */
	gchar		*label_text,		/* Copy of the label text */
					*accel_text;		/* Copy of accelkey label text */
	const guint8	**icon_data;		/* Shared icon data for the pixmap */
	guint		accel_key,
					accel_mods;
} GUIMenuItemData;
...
static gint GUIBlockInputEventCB(
	GtkWidget *widget, GdkEvent *event, gpointer data
);

/* Dynamic Data Exchange Callbacks */
static void GUISelectionDataDelete(GUISelectionData *sel_data);
...
/* Drag & Drop Drag Icon Callbacks */
static void GUIDNDDragIconDestroyCB(gpointer data);
...
/* Banner Callbacks */
static void GUIBannerDestroyCB(gpointer data);
static void GUIBannerRealizeCB(GtkWidget *widget, gpointer data);
...
gboolean GUICListGetCellGeometry(
	GtkCList *clist,
	const gint column, const gint row,
	gint *x, gint *y, gint *width, gint *height
);

/* Display */
gpointer GUIGetDisplay(void);

/* Root/Desktop */
GdkWindow *GUIGetWindowRoot(void);
...
/* GtkCTreeNode Utilities */
GtkCTreeNode *GUICTreeNodeGetParent(GtkCTreeNode *node);
GtkCTreeNode *GUICTreeNodeGetChild(GtkCTreeNode *node);
GtkCTreeNode *GUICTreeNodeGetSibling(GtkCTreeNode *node);
gboolean GUICTreeNodeGetExpanded(GtkCTreeNode *node);
gint GUICTreeNodeRow(
	GtkCTree *ctree,
	GtkCTreeNode *node
);
...
void GUIGetTextBounds(
	GdkFont *font,
	const gchar *text, const gint text_length,
...
/* GdkGC */
GdkGC *GUIGCNew(GdkWindow *window);
GdkGC *GUIGCUnref(GdkGC *gc);
...
/* GdkEvents */
void GUIEventsProcessThrough(void);
void GUIEventsProcessCurrent(void);
void GUIEventsProcessUntil(const gulong limit_ms);


/* Timeout & Idle */
guint GUITimeoutRemove(const guint id);
guint GUIIdleRemove(const guint id);
...
/* GdkFont */
GdkFont *GUIFontUnref(GdkFont *font);


/* GdkColormap */
GdkColormap *GUIColormapUnref(GdkColormap *colormap);


/* GdkBitmap */
GdkBitmap *GUIBitmapNew(
	GdkWindow *window,
...
GdkBitmap *GUIBitmapUnref(GdkBitmap *bitmap);

/* GdkPixmap */
GdkPixmap *GUIPixmapNew(
	GdkWindow *window,
	const gint width, const gint height,
	const gint depth
);
GdkPixmap *GUIPixmapNewFromXPMData(
	GdkWindow *window,
	GdkBitmap **mask_rtn,
	GdkColor *bg_color,
	guint8 **data
);
GdkPixmap *GUIPixmapNewFromXPMFile(
	GdkWindow *window,
	GdkBitmap **mask_rtn,
	GdkColor *bg_color,
	const gchar *path
);
GtkWidget *GUIGtkPixmapNewFromXPMData(
	GdkWindow *window,
	GdkColor *bg_color,
	guint8 **data
);
GtkWidget *GUICreateMenuItemIcon(guint8 **icon_data);
GdkPixmap *GUIPixmapUnref(GdkPixmap *pixmap);


/* GtkObjects */
GtkObject *GUIObjectRef(GtkObject *o);
GtkObject *GUIObjectUnref(GtkObject *o);
...
void GUIDNDClearDragIcon(GtkWidget *w);
...
GtkWidget *GUIBannerCreate(
	...
	const gint inside_border,
	const gint outside_border
);
...
void GUIMenuItemSetLabel(
	GtkWidget *menu_item,
	const gchar *label
);
GtkWidget *GUIMenuItemGetLabelWidget(GtkWidget *menu_item);
void GUIMenuItemSetPixmap(
	GtkWidget *menu_item,
	guint8 **icon_data
);
...
static gchar *G_STRCAT(gchar *sa, const gchar *sb)
{
	if(sa != NULL) {
		if(sb != NULL) {
			gchar *sc = g_strconcat(sa, sb, NULL);
			if(sc != NULL)
			{
				g_free(sa);
				sa = sc;
...
/*
 *	Block input any GdkEvent "stealer" callback.
 */
static gint GUIBlockInputEventCB(
	GtkWidget *widget, GdkEvent *event, gpointer data
)
{
	...
	/* Get the GtkWidget that this event occured over */
	ew = gtk_get_event_widget(event);
	if(ew != NULL)
	{
		/* Did this event not occure over the blocked GtkWidget? */
		GtkWidget *ew_toplevel = gtk_widget_get_toplevel(ew);
		if(ew_toplevel != d->w)
		{
			/* Pass this event on to its intended widget */
			status = gtk_widget_event(ew, event);
		}
	}
...
static void GUIDDEOwnerDestroyCB(gpointer data)
{
	GUISelectionOwner *owner = GUI_SELECTION_OWNER(data);
	if(owner == NULL)
		return;

	/* Delete the selection data */
	if(owner->datas_list != NULL)
	{
		g_list_foreach(
			owner->datas_list,
			(GFunc)GUISelectionDataDelete,
			NULL
		);
		g_list_free(owner->datas_list);
	}

	/* Do not disconnect the signal functions, they are already
	 * disconnected when the GtkWidget was destroyed
	 */
...
{
	GUISelectionOwner *owner = GUI_SELECTION_OWNER(data);
	if((widget == NULL) || (owner == NULL))
		return(FALSE);

	/* Clear the selection data */
	if(event != NULL)
	{
		const GdkAtom selection_atom = event->selection;
		GList *glist;
		GUISelectionData *sel_data;
		for(glist = owner->datas_list;
			glist != NULL;
			glist = g_list_next(glist)
		)
		{
			sel_data = GUI_SELECTION_DATA(glist->data);
			if(sel_data == NULL)
				continue;

			/* Is this the selection data to clear? */
			if(sel_data->selection_atom == selection_atom)
			{
				sel_data->target_atom = GDK_NONE;
				sel_data->type_atom = GDK_NONE;
				g_free(sel_data->data);
				sel_data->data = NULL;
				sel_data->length = 0;
			}
...
)
{
	...
	/* Get the selection data by the specified selection */
	data_sent = FALSE;
	for(glist = owner->datas_list;
		glist != NULL;
		glist = g_list_next(glist)
	)
	{
		...
		/* Is this the requested selection and target data? */
		if((selection_data->selection == sel_data->selection_atom) &&
		   (selection_data->target == sel_data->target_atom)
		)
		{
...
{
	GdkAtom type;
	GUISelectionData *sel_rtn = GUI_SELECTION_DATA(data);
	if((widget == NULL) || (selection_data == NULL) ||
	   (sel_rtn == NULL)
	)
		return;
	...
	/* Mark that the selection was received */
	sel_rtn->length = 0;
	...
	/* Selection or type mismatch? */
	if((selection_data->selection != sel_rtn->selection_atom) ||
	   (selection_data->target != sel_rtn->target_atom)
	)
		return;
	 ...
	 * by its type
	 */
	sel_rtn->type_atom = type = selection_data->type;

	/* Atoms List */
	if(type == GDK_SELECTION_TYPE_ATOM)
	{
		gint	i,
					m = selection_data->length / sizeof(GdkAtom);
		gchar	*s = NULL,
					*name;
		...
		sel_rtn->type_atom = GDK_SELECTION_TYPE_STRING;
	}
	/* GdkBitmap */
...
/* TODO */
	...
	else if(type == GDK_SELECTION_TYPE_STRING)
	{
		 ...
		 */
		const gint len = selection_data->length;
...
/*
 *	DND Drag Icon "destroy" signal callback.
 */
static void GUIDNDDragIconDestroyCB(gpointer data)
{
	GUIDNDDragIcon *dnd_drag_icon = GUI_DND_DRAG_ICON(data);
	if(dnd_drag_icon == NULL)
		return;

	if(dnd_drag_icon->ref_count > 0)
		g_printerr(
"GUIDNDDragIconDestroyCB(): Warning: Destroying dnd_drag_icon=%p\
 with %i reference counts remaining.\n",
			dnd_drag_icon,
			dnd_drag_icon->ref_count
		);

	GTK_WIDGET_DESTROY(dnd_drag_icon->toplevel);

	g_free(dnd_drag_icon);
}
...
{
	GUIDNDDragIcon *dnd_drag_icon;
	GUIDNDSourceData *d = GUI_DND_SOURCE_DATA(data);
	if((widget == NULL) || (dc == NULL) || (d == NULL))
		return;

	dnd_drag_icon = GUI_DND_DRAG_ICON(gtk_object_get_data(
		GTK_OBJECT(widget),
		GUI_DND_DRAG_ICON_KEY
	));
	if(dnd_drag_icon != NULL)
	{
		dnd_drag_icon->ref_count++;
		...
		if(dnd_drag_icon->toplevel != NULL)
			gtk_drag_set_icon_widget(
				dc,
				dnd_drag_icon->toplevel,
				dnd_drag_icon->x,
				dnd_drag_icon->y
			);
	}
...
static void GUIDNDTargetDataDestroyCB(gpointer data)
{
	...
	d->highlight_draw_sigid = GTK_IDLE_REMOVE(d->highlight_draw_sigid);
	d->unhighlight_draw_sigid = GTK_IDLE_REMOVE(d->unhighlight_draw_sigid);
...
static void GUIBannerDestroyCB(gpointer data)
{
	...
	GDK_GC_UNREF(banner->gc);

	if(banner->colormap != NULL)
	{
		GdkColormap *colormap = banner->colormap;

		GDK_COLORMAP_FREE_COLOR(colormap, banner->color_fg);
		g_free(banner->color_fg);
		GDK_COLORMAP_FREE_COLOR(colormap, banner->color_bg);
		g_free(banner->color_bg);
		GDK_COLORMAP_FREE_COLOR(colormap, banner->color_border);
		g_free(banner->color_border);

		(void)GDK_COLORMAP_UNREF(colormap);
	}

	(void)GDK_FONT_UNREF(banner->font);
...
/*
 *	Banner "realize" signal callback.
 */
static void GUIBannerRealizeCB(GtkWidget *widget, gpointer data)
{
	GdkWindow *window;
	GUIBannerData *banner = GUI_BANNER_DATA(data);
	if((widget == NULL) || (banner == NULL))
		return;

	window = widget->window;

	if(banner->colormap == NULL)
		banner->colormap = GDK_COLORMAP_REF(gtk_widget_get_colormap(widget));

	if(banner->colormap != NULL)
	{
		GdkColormap *colormap = banner->colormap;
		if(banner->color_fg != NULL)
			GDK_COLORMAP_ALLOC_COLOR(colormap, banner->color_fg);
		if(banner->color_bg != NULL)
			GDK_COLORMAP_ALLOC_COLOR(colormap, banner->color_bg);
		if(banner->color_border != NULL)
			GDK_COLORMAP_ALLOC_COLOR(colormap, banner->color_border);
	}

	if(banner->gc == NULL)
		banner->gc = GUIGCNew(window);
}
...
/*
 *	Local menu item activate callback.
 */
static void GUIMenuItemActivateCB(gpointer data)
{
...
/*
 *	GUIMenuItemActivateData "destroy" callback.
 */
static void GUIMenuItemActivateDataDestroyCB(gpointer data)
{
	GUIMenuItemActivateData *d = GUI_MENU_ITEM_ACTIVATE_DATA(data);
...
/*
 *	GUIMenuItemData "destroy" callback.
 */
static void GUIMenuItemDataDestroyCB(gpointer data)
{
...
/*
 *	Gets the root (desktop) GdkWindow.
 */
GdkWindow *GUIGetWindowRoot(void)
{
...
/*
 *	Gets the GdkWindow's coordinates relative to the desktop.
 */
void GUIGetWindowRootPosition(
	GdkWindow *window,
	gint *x_rtn, gint *y_rtn
)
{
	...
	if(gui_window_root == NULL)
		 gui_window_root = GUIGetWindowRoot();
...
/*
 *	Converts the GdkWindow's coordinates to be relative to the
 *	desktop.
 */
void GUIGetPositionRoot(
	GdkWindow *w,
...
/*
 *	Gets the GtkCTreeNode's parent.
 */
GtkCTreeNode *GUICTreeNodeGetParent(GtkCTreeNode *node)
{
	GtkCTreeRow *row_ptr;
	...
	return(row_ptr->parent);
}

/*
 *	Gets the GtkCTreeNode's first child.
 */
GtkCTreeNode *GUICTreeNodeGetChild(GtkCTreeNode *node)
{
	GtkCTreeRow *row_ptr;
	...
	return(row_ptr->children);
}

/*
 *	Gets the GtkCTreeNode's sibling.
 */
GtkCTreeNode *GUICTreeNodeGetSibling(GtkCTreeNode *node)
{
	GtkCTreeRow *row_ptr;
	...
	return(row_ptr->sibling);
}

/*
 *	Gets the GtkCTreeNode's expanded flag.
 */
gboolean GUICTreeNodeGetExpanded(GtkCTreeNode *node)
{
	GtkCTreeRow *row_ptr;
	...
	return(row_ptr->expanded);
}

/*
 *	Gets the row index of the GtkCTreeNode.
 */
gint GUICTreeNodeRow(
	GtkCTree *ctree,
	GtkCTreeNode *node
...
/*
 *	Called by GUICTreeNodeDeltaRows() to count the number of rows
 *	to the end node.
 */
static void GUICTreeNodeDeltaRowsIterate(
	GtkCTree *ctree,
...
/*
 *	Gets the number of rows between the GtkCTreeNodes.
 *
 *	The start specifies the starting GtkCTreeNode which may be
 *	NULL for the first toplevel GtkCTreeNode.
 */
gint GUICTreeNodeDeltaRows(
	GtkCTree *ctree,
	GtkCTreeNode *start,
...
{
//#define USE_GTK_WINDOW_TO_BLOCK
	GtkWidget *bw;				/* Block input GtkWidget */
	GUIBlockInputData *d;
	...
	if(d == NULL)
	{
...
#ifdef USE_GTK_WINDOW_TO_BLOCK
		d->invisible = bw = gtk_window_new(GTK_WINDOW_POPUP);
#else
		d->invisible = bw = gtk_invisible_new();
#endif
		d->w = w;
		d->level = 0;

		gtk_widget_set_usize(bw, 1, 1);
		gtk_widget_set_uposition(bw, -1, -1);
		...
		gtk_signal_connect(
			GTK_OBJECT(bw), "key_press_event",
			GTK_SIGNAL_FUNC(GUIBlockInputEventCB), d
		);
		gtk_signal_connect(
			GTK_OBJECT(bw), "key_release_event",
			GTK_SIGNAL_FUNC(GUIBlockInputEventCB), d
		);
		gtk_signal_connect(
			GTK_OBJECT(bw), "button_press_event",
			GTK_SIGNAL_FUNC(GUIBlockInputEventCB), d
		);
		gtk_signal_connect(
			GTK_OBJECT(bw), "button_release_event",
			GTK_SIGNAL_FUNC(GUIBlockInputEventCB), d
		);
		gtk_signal_connect(
			GTK_OBJECT(bw), "motion_notify_event",
			GTK_SIGNAL_FUNC(GUIBlockInputEventCB), d
		);
		gtk_signal_connect(
			GTK_OBJECT(bw), "enter_notify_event",
			GTK_SIGNAL_FUNC(GUIBlockInputEventCB), d
		);
		gtk_signal_connect(
			GTK_OBJECT(bw), "leave_notify_event",
			GTK_SIGNAL_FUNC(GUIBlockInputEventCB), d
		);
	}

	/* Get the GtkInvisible that will be used to grab all input */
	bw = d->invisible;
	if(bw == NULL)
		return;
	...
	if(block)
	{
		/* Increase the block input level and check if it is
		 * the first blocking level
		 */
		d->level++;
		if(d->level == 1)
		{
			/* This is the first blocking level so we
			 * need to map the GtkInvisible and grab it so
			 * that input will be blocked
			 */
#ifdef USE_GTK_WINDOW_TO_BLOCK
			gtk_window_set_modal(GTK_WINDOW(bw), TRUE);
			gtk_widget_show(bw);
#else
			gtk_widget_show(bw);
			if(gtk_grab_get_current() != bw)
				gtk_grab_add(bw);
#endif
		}
	}
	else
	{
		...
		{
#ifdef USE_GTK_WINDOW_TO_BLOCK
			gtk_window_set_modal(GTK_WINDOW(bw), FALSE);
			gtk_widget_hide(bw);
#else
			if(gtk_grab_get_current() == bw)
				gtk_grab_remove(bw);
		...
		{
			g_printerr(
"GUIBlockInput(): GtkWidget %p: Block level underflow (level %i).\n",
				w,
				d->level
			);
			d->level = 0;
...
/*
 *	Processes GdkEvents by calling gtk_main_iteration() until
 *	there are no more pending GdkEvents.
 *
 *	Warning, time out callbacks and idle callbacks may be
 *	called during this call and produce a race condition. If
 *	the processing time for the callbacks is greater than
 *	the delay between the calls then this function may never
 *	return.
 */
void GUIEventsProcessThrough(void)
{
...
/*
 *	Processes only the GdkEvents schedualed at the start of this
 *	call by calling gtk_main_iteration().
 */
void GUIEventsProcessCurrent(void)
{
...
/*
 *	Processes GdkEvents by calling gtk_main_iteration() until
 *	there are no more pending GdkEvents or the time limit has
 *	been reached.
 *
 *	The limit_ms specifies the time limit in milliseconds.
 */
void GUIEventsProcessUntil(const gulong limit_ms)
{
	gint            i = gtk_events_pending(),
	...
	while(i > 0)
	{
		...
		/* More events have arrived than the number calculated to
		 * have been processed based on the number of events at
		 * the start of this call?
		 */
		if((i - i2) <= 0)
		{
			/* If a timer has been schedualed then check if we
			 * have exceeded the specified time limit
			 */
			if(timer != NULL)
			{
				if(G_TIMER_ELAPSED_MS(timer) > limit_ms)
					break;
			}
			else
			{
				/* No timer schedualed yet so schedual one now */
				timer = g_timer_new();
				g_timer_start(timer);
			}
		}
...
)
{
	...
	/* Set the GdkWindow's WM icon
	 *
	 * gdk_window_set_icon() will unref() the pixmap and mask
	 */
	gdk_window_set_icon(
		window,
		NULL,				/* No icon GdkWindow */
		pixmap, mask
	);
}

/*
 *	Sets the WM icon for the GdkWindow from an XPM file.
 */
void GUISetWMIconFile(
	GdkWindow *window,
	const gchar *path
...
{ 
	...
	if((window == NULL) || STRISEMPTY(path))
		return;
...
if(rcstyle == NULL)			\
 rcstyle = gtk_rc_style_new();		\
	...
	/* Must realize the GtkWindow so that we can set its GdkWindow */
...
/*
 *	Creates a new GdkBitmap from RGBA image data using its alpha
 *	channel.
 *
 *	The width and height specifies the size of the RGBA image data
 *	and the size of the new GdkBitmap.
 *
 *	The bpl specifies the bytes per line of the RGBA image data.
 *
 *	The rgba specifies the RGBA image data.
 *
 *	The threshold specifies the alpha channel threshold, any alpha
 *	channel value greater than or equal to threshold will be
 *	converted to a 1 bit on the GdkBitmap and any alpha channel
 *	value less than threshold will be converted to a 0 but on the
 *	GdkBitmap.
 *
 *	The window specifies the GdkWindow to create the GdkBitmap.
 *	The GdkWindow may not be a GdkPixmap or GdkBitmap. If window
 *	is NULL then the root GdkWindow will be used.
 */
GdkBitmap *GUICreateBitmapFromDataRGBA(
	const gint width, const gint height,
	const gint bpl,
...
)
{
	...
	/* Calculate the number of bytes that are needed to hold
	 * one row of bits for the bitmap data
	 */
	xbm_bpl = (width / 8) + (((width % 8) != 0) ? 1 : 0);
	...
	/* Convert the RGBA data to the bitmap data */
	src_data = rgba;
	tar_data = xbm_data;
	tar_bpl = xbm_bpl;
	for(y = 0; y < height; y++)
	{
		src_ptr = src_data + (y * src_bpl);
		tar_row = tar_data + (y * tar_bpl);
		for(x = 0; x < width; x++)
		{
			if(src_ptr[3] >= threshold)
				tar_row[(x / 8)] |= (guint8)(1 << (x % 8));
			src_ptr += bpp;
		}
	}

	/* Create the GdkBitmap from the bitmap data */
	bitmap = gdk_bitmap_create_from_data(
		window,
		(const gchar *)xbm_data,
		width, height
	);
...
GtkWidget *GUICreateMenuItemIcon(guint8 **icon_data)
{
	...
	if(icon_data != NULL)
	{
		...
		GdkBitmap *mask;
		GdkPixmap *pixmap = GUIPixmapNewFromXPMData(
		    NULL,
			&mask,
			NULL,
		    icon_data
...
)
{
	...
	/* Show/hide the icon GtkPixmap's toplevel boxt because it may
	 * be holding multiple icons that need to be shown/hidden
	 */
	w = d->icons_toplevel;
	if(w != NULL)
	{
...
/*
 *	Underlines the character in a button's label.
 */
void GUIButtonLabelUnderline(
	GtkWidget *w,
	const guint c
...
)
{
	...
	)
	{
		...
		if(pixmap != NULL)
		{
			...
			/* Record the new icon data pointer */
			d->icon_data = (const guint8 **)icon;

			/* Set the new icon pixmap */
			w = d->pixmap;
			if(w != NULL)
			{
...
)
{
	...
	if(icon != NULL)
	{
		...
		if(pixmap != NULL)
		{
			...
			d->icons_toplevel = w = gtk_hbox_new(TRUE, 0);
			gtk_box_pack_start(GTK_BOX(parent), w, TRUE, FALSE, border);
			gtk_widget_show(w);
			parent2 = w;

			/* Icon GtkBox for client parenting of additional
			 * adjacent icons
			 */
			w = gtk_hbox_new(FALSE, 0);
...
"GUIDDESetDirect(): Failed:\
 GtkWidget %p does not have a GdkWindow.\n",
	...
	owner = GUI_SELECTION_OWNER(gtk_object_get_data(
		GTK_OBJECT(w),
		GUI_SELECTION_OWNER_KEY
	));
	if(owner == NULL)
	{
		/* The owner data was not set, create it */
		owner = GUI_SELECTION_OWNER(g_malloc0(sizeof(GUISelectionOwner)));
		if(owner == NULL)
		{
			g_free(data);
			return;
		...
		gtk_object_set_data_full(
			GTK_OBJECT(w), GUI_SELECTION_OWNER_KEY,
			owner, GUIDDEOwnerDestroyCB
		);
		owner->w = w;
		owner->selection_clear_event_sigid = gtk_signal_connect(
			GTK_OBJECT(w), "selection_clear_event",
			GTK_SIGNAL_FUNC(GUIDDESelectionClearEventCB), owner
		);
		owner->selection_get_sigid = gtk_signal_connect(
			GTK_OBJECT(w), "selection_get",
			GTK_SIGNAL_FUNC(GUIDDESelectionGetCB), owner
		);
	}

	/* Get the selection data based on the specified selection */
	sel_data = NULL;
	for(glist = owner->datas_list;
		glist != NULL;
		glist = g_list_next(glist)
	...
	if(glist == NULL)
	{
		sel_data = GUI_SELECTION_DATA(g_malloc0(sizeof(GUISelectionData)));
		if(sel_data == NULL)
		{
			g_free(data);
			return;
		...
		sel_data->selection_atom = selection_atom;

		owner->datas_list = g_list_append(
			owner->datas_list,
			sel_data
		);
	}
	...
	sel_data->target_atom = target_atom;
	sel_data->type_atom = type_atom;

	/* Delete the owner's existing data and transfer the new data */
	g_free(sel_data->data);
	sel_data->data = data;
	sel_data->length = length;
...
"GUIDDEGet(): Failed:\
 GtkWidget %p does not have a GdkWindow.\n",
	...
	/* Create the selection data return */
	sel_data = GUI_SELECTION_DATA(g_malloc0(sizeof(GUISelectionData)));
	if(sel_data == NULL)
		return(NULL);
	...
	sel_data->type_atom = GDK_NONE;
	sel_data->data = NULL;
	sel_data->length = -1;			/* -1 means data not received */
	...
	selection_received_sigid = gtk_signal_connect(
		...
		GTK_SIGNAL_FUNC(GUIDDESelectionReceivedCB), sel_data
	);
	...
	))
	{
		...
		GUISelectionDataDelete(sel_data);
		return(NULL);
	}
	 ...
	 * immediately
	 */
	while(sel_data->length < 0)
		gtk_events_process();

	/* Disconnect the "selection_received" signal */
	gtk_signal_disconnect(
		GTK_OBJECT(w),
		selection_received_sigid
	);

	/* Got the selection, transfer the selection return's data to
	 * the return data
	 */
	data = (guint8 *)sel_data->data;
	if(length != NULL)
		*length = sel_data->length;
	sel_data->data = NULL;
	sel_data->length = 0;

	/* Delete the selection return data */
	GUISelectionDataDelete(sel_data);
...
)
{
	...
	/* Add one more line to describe the total number of items? */
	if((nitems > -1) ? (nitems > nitems_in_list) : FALSE)
	{
		const gchar *text = "... 123456789 items total";
	...
	mask_gc = GUIGCNew((GdkWindow *)mask);
	if(mask_gc == NULL)
	{
		GDK_PIXMAP_UNREF(pixmap);
		GDK_BITMAP_UNREF(mask);
...
/*
 *	Sets the GtkWidget's DND drag icon.
 *
 *	The w specifies the GtkWidget.
 *
...
)
{
	...
	if(dnd_drag_icon == NULL)
	{
		dnd_drag_icon = GUI_DND_DRAG_ICON(g_malloc0(
			sizeof(GUIDNDDragIcon)
		));
		if(dnd_drag_icon == NULL)
			return;

		gtk_object_set_data_full(
			GTK_OBJECT(w), GUI_DND_DRAG_ICON_KEY,
			dnd_drag_icon, GUIDNDDragIconDestroyCB
		);
	}

#if 0
/* This can occur normally when a drag icon needs to be changed
 * during a drag
 */
	if(dnd_drag_icon->ref_count > 0)
		g_printerr(
"GUIDNDSetDragIcon(): Warning: Setting dnd_drag_icon=%p with\
 %i reference counts remaining.\n",
			dnd_drag_icon,
			dnd_drag_icon->ref_count
		);
#endif
	...
	if(pixmap == NULL)
	{
		...
		GTK_WIDGET_DESTROY(dnd_drag_icon->toplevel);
		memset(dnd_drag_icon, 0x00, sizeof(GUIDNDDragIcon));
		return;
	}
	 ...
	 */
	w = dnd_drag_icon->toplevel;
	if(w == NULL)
	{
		/* Create the DND drag icon GtkWidget */
		GtkWidget *parent;

		dnd_drag_icon->toplevel = w = gtk_window_new(GTK_WINDOW_POPUP);
		gtk_window_set_policy(GTK_WINDOW(w), FALSE, FALSE, FALSE);
		parent = w;

		dnd_drag_icon->icon_pm = w = gtk_pixmap_new(pixmap, mask);
		gtk_container_add(GTK_CONTAINER(parent), w);
		gtk_widget_show(w);
		is_pixmap_set = TRUE;
	}
	...
	/* Set the hot spot */
	dnd_drag_icon->x = hot_x;
	dnd_drag_icon->y = hot_y;

	/* Set the size of the new DND drag icon */
	gdk_window_get_size(
		pixmap,
		&dnd_drag_icon->width, &dnd_drag_icon->height
	);
	...
	if(!is_pixmap_set)
		gtk_pixmap_set(
			GTK_PIXMAP(dnd_drag_icon->icon_pm),
			pixmap, mask
		);
}

/*
 *	Clears the GtkWidget's DND drag icon.
 *
 *	The w specifies the GtkWidget.
 */
void GUIDNDClearDragIcon(GtkWidget *w)
...
)
{
	...
	banner->font = GDK_FONT_REF(font);
	if(color_fg != NULL)
		banner->color_fg = g_memdup(color_fg, sizeof(GdkColor));
	if(color_bg != NULL)
		banner->color_bg = g_memdup(color_bg, sizeof(GdkColor));
	if(color_border != NULL)
		banner->color_border = g_memdup(color_border, sizeof(GdkColor));
	banner->justify = justify;
	banner->inside_border = inside_border;
	banner->outside_border = outside_border;

	w = banner->toplevel;
	gtk_object_set_data_full(
		GTK_OBJECT(w), GUI_BANNER_DATA_KEY,
		banner, GUIBannerDestroyCB
	);
	gtk_widget_add_events(
		w,
	...
	gtk_signal_connect(
		...
		GTK_SIGNAL_FUNC(GUIBannerRealizeCB), banner
	);
	gtk_signal_connect(
	...
	if(expand)
		...
		width = gdk_string_measure(font, label) +
			(2 * inside_border) + (2 * outside_border);
	else
		width = -1;
	height = ((font != NULL) ? (font->ascent + font->descent) : 1) +
		(2 * inside_border) + (2 * outside_border);
	gtk_widget_set_usize(w, width, height);
...
void GUIBannerDraw(GtkWidget *w)
{
	...
	inside_border = banner->inside_border;
	outside_border = banner->outside_border;
	font = banner->font;
	...
	gdk_draw_rectangle(
		...
		0 + outside_border,
		0 + outside_border,
		width - (2 * outside_border),
		height - (2 * outside_border)
	);
		...
		switch(justify)
		{
		  ...
		  case GTK_JUSTIFY_RIGHT:
			gdk_draw_string(
				...
				font,
				gc,
				(width - swidth - (2 * outside_border) - (2 * inside_border)) - lbearing,
				((height - (font->ascent + font->descent)) / 2) +
					font->ascent,
				label
			);
		  ...
		  case GTK_JUSTIFY_LEFT:
			gdk_draw_string(
				...
				font,
				gc,
				((2 * outside_border) + (2 * inside_border)) - lbearing,
				((height - (font->ascent + font->descent)) / 2) +
					font->ascent,
				label                    
			);
...
)
{
	...
	mi = GUI_MENU_ITEM_DATA(g_malloc0(sizeof(GUIMenuItemData)));
	if(mi == NULL)
		return(NULL);
	...
	switch(type)
	{
	  ...
	  case GUI_MENU_ITEM_TYPE_SUBMENU:
		...
		gtk_widget_show(w);
		/* GtkTable for the icon GtkPixmap, main GtkLabel, and
		 * accelkey GtkLabel
		 */
		ncolumns = 3;
		nrows = 1;
		...
		/* GtkAlignment for the msin GtkLabel */
		...
		parent2 = w;
		/* Create the GtkLabel */
		mi->label_text = g_strdup((label != NULL) ? label : "");
		mi->label = w = gtk_label_new(mi->label_text);
		gtk_label_set_justify(GTK_LABEL(w), GTK_JUSTIFY_LEFT);
		gtk_container_add(GTK_CONTAINER(parent2), w);
		gtk_widget_show(w);

		/* Set the callback data and signals for this GtkMenuItem */
		if(func_cb != NULL)
		{
		...
		/* GtkAlignment for the accelkey GtkLabel */
		...
		gtk_widget_show(w);
		parent2 = w;
		/* Create the accelkey GtkLabel */
		mi->accel_label = w = gtk_label_new("");
		gtk_label_set_justify(GTK_LABEL(w), GTK_JUSTIFY_RIGHT);
		gtk_container_add(GTK_CONTAINER(parent2), w);
		/* Set the accelkey GtkLabel */
	  ...
	  case GUI_MENU_ITEM_TYPE_CHECK:
		...
		mi->menu_item = menu_item = parent = w = gtk_check_menu_item_new();
		gtk_widget_add_events(
			w,
			GDK_ENTER_NOTIFY_MASK | GDK_LEAVE_NOTIFY_MASK
		);
		...
		/* Create the icon GtkPixmap only if an icon was specified
		 * for GtkCheckMenuItems
		 */
		if(icon != NULL)
		{
			mi->icon_data = (const guint8 **)icon;
		...
		else
		{
			gtk_widget_set_usize(
				w,
				-1,
				GUI_MENU_ITEM_DEF_HEIGHT + 2
			);
		}

		/* GtkAlignment for the main GtkLabel */
		w = gtk_alignment_new(0.0f, 0.5f, 0.0f, 0.0f);
		gtk_table_attach(
			GTK_TABLE(parent),
			w,
		...
		/* Set the callback data and signals for this GtkCheckMenuItem */
		if(func_cb != NULL)
		{
			GUIMenuItemActivateData *d = GUI_MENU_ITEM_ACTIVATE_DATA(g_malloc0(
				sizeof(GUIMenuItemActivateData)
	  ...
	  case GUI_MENU_ITEM_TYPE_SEPARATOR:
		...
		GTK_WIDGET_UNSET_FLAGS(GTK_WIDGET(w), GTK_SENSITIVE);
		gtk_widget_show(w);

 	    /* Create a GtkHSeparator */
...
)
{
	...
	if(label != NULL)
	{
		...
		/* Record the new label text */
		g_free(d->label_text);
		d->label_text = g_strdup(label);
...
/*
 *	Gets the GtkMenuItem's main GtkLabel.
 */
GtkWidget *GUIMenuItemGetLabelWidget(GtkWidget *menu_item)
{
	GUIMenuItemData *d;
...
)
{
	...
	if(accelgrp != NULL)
	{
		/* Remove the existing accelerator key? */
		if(d->accel_key != 0x00000000)
	...
	if(w != NULL)
	{
		...
		if(accel_key != 0x00000000)
		{
			const gchar *key_name;
			gchar *accel_text = g_strdup("");
...
)
{
	...
	if(emit_signal)
		...
		GTK_CHECK_MENU_ITEM(w)->active = active;
}
